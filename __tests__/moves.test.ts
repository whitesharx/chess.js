...
test('moves', () => {
  ...
  expect(new Set(chess.moves())).toEqual(
    new Set([
      'a3',
      'a4',
      'b3',
...
test('moves - single square', () => {
  const chess = new Chess()
  expect(new Set(chess.moves({ square: 'e2' }))).toEqual(new Set(['e3', 'e4']))
})

test('moves - single square - invalid square', () => {
  const chess = new Chess()
  expect(chess.moves({ square: 'e9' })).toEqual([])
})
...
test('moves - single square - pinned piece', () => {
  ...
  expect(chess.moves({ square: 'c3' })).toEqual([])
})

test('moves - single square - promotion', () => {
  const chess = new Chess('8/k7/8/8/8/8/7p/K7 b - - 0 1')
  expect(new Set(chess.moves({ square: 'h2' }))).toEqual(
    new Set(['h1=N', 'h1=B', 'h1=R+', 'h1=Q+']),
  )
})
...
test('moves - single square - castling', () => {
  ...
  expect(new Set(chess.moves({ square: 'e1' }))).toEqual(
    new Set(['Kf1', 'Kd1', 'O-O', 'O-O-O']),
  )
})
...
test('moves - single square - trapped king', () => {
  ...
  expect(new Set(chess.moves({ square: 'a3' }))).toEqual(new Set([]))
})
